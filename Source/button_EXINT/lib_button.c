...
void BUTTON_init(void) {
  ...
  LPC_SC->EXTMODE = 0x7;  /* Setto la modalità dei button su edge-sensitive*/
	

  NVIC_EnableIRQ(EINT2_IRQn);              /* Abilito le interruzioni tramite IC per KEY2 */
	NVIC_SetPriority(EINT2_IRQn, 1);				 /* Setto la priorità per KEY2 */
  NVIC_EnableIRQ(EINT1_IRQn);              /* Abilito le interruzioni trmaite IC per KEY1  */
	NVIC_SetPriority(EINT1_IRQn, 2);				 /* Setto la priorità per KEY1 */
  NVIC_EnableIRQ(EINT0_IRQn);              /* Abilito le interruzioni tramite IC per INT0*/
	NVIC_SetPriority(EINT0_IRQn, 3);				 /* Setto la priorità per INT0 */
